...
HEADERS = {
    ...
    'Accept-Encoding': 'gzip, deflate, br', # Дозволяємо сайту надсилати стиснений контент
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
    'Connection': 'keep-alive',
    'Upgrade-Insecure-Requests': '1',
    'DNT': '1' # Do Not Track
}
OUTPUT_FILE = "server_equipment_data.xlsx"
all_products_data = []
...
def get_category_name_from_page(soup, base_url):
    ...
    try:
        ...
        if 'product-category' in url_parts:
            ...
            # Спробувати взяти останній значущий сегмент після product-category
            # /ua/product-category/server-equipment-ua/servers-ua/ -> servers-ua
            # /ua/product-category/server-equipment-ua/tape-systems-ua/tape-cartridges-ua/ -> tape-cartridges-ua
            relevant_parts = [part for part in url_parts[idx+1:] if part and part != 'page']
            if relevant_parts:
                category_candidate = relevant_parts[-1] # Останній елемент
...
for base_url in BASE_URLS:
    ...
    while True:
        ...
        try:
            ...
            # --- Блок для налагодження (розкоментуйте, якщо все ще не працює) ---
            # filename = f"debug_page_content_{base_url.strip('/').split('/')[-2]}_page_{current_page}.html"
            # with open(filename, "w", encoding="utf-8") as f:
            #     f.write(response.text)
            # print(f"    DEBUG: HTML для {page_url} збережено у файл {filename}. Статус: {response.status_code}")
        ...
        # Змінено селектор: шукаємо безпосередньо div.product__item
        ...
        for product_el in product_cards: # product_el тепер це безпосередньо div.product__item
            product_data = {
                "Назва": "N/A",
                "Виробник": "N/A",
                "Ціна": "N/A",
            ...
            # Виробник (шукаємо всередині div.product__item)
            ...
            # Назва (шукаємо всередині div.product__item)
            ...
            if title_tag:
                ...
                if '|' in full_title_str and product_data["Виробник"] == "N/A":
                    title_parts = [p.strip() for p in full_title_str.split('|')]
                    if len(title_parts) >= 2:
                        potential_manufacturer_from_title = title_parts[1] if len(title_parts) > 1 else title_parts[0]
                        if potential_manufacturer_from_title:
                             product_data["Виробник"] = potential_manufacturer_from_title
            else:
            ...
            # Ціна (шукаємо всередині div.product__item)
        ...
        if parsed_products_on_this_page_count == 0 and len(product_cards) > 0:
            ...
            # Не перериваємо цикл тут, щоб спробувати наступну сторінку

        current_page += 1
        time.sleep(1.2) # Трохи збільшено затримку
...
if all_products_data:
    ...
    print("Перевірте вивід консолі на наявність помилок або повідомлень про відсутність товарів.")
    print("Якщо ви впевнені, що товари мають бути, але скрипт їх не бачить, розкоментуйте блок для налагодження HTML у коді та перевірте збережені файли.")
